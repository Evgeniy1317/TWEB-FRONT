...
export default function ProfilePage() {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'admin') {
      ...
      const canSetCancelled = isAdmin || isManager;
              ...
              {sortedAllOrders.map(order => (
                <article key={order.id} className="border-2 border-black bg-white p-5 sketch-shadow sm:p-6">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <button
                      type="button"
                      disabled={!canSetCancelled || adminUpdatingOrderId === order.id}
                      onClick={() => void setStatusForOrder(order.id, 'cancelled')}
                      className={
                        order.status === 'cancelled'
                          ? 'border-2 border-black bg-red-100 px-3 py-2 text-xs font-black uppercase tracking-wide text-red-700 transition-colors hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-45'
                          : 'border-2 border-black bg-white px-3 py-2 text-xs font-black uppercase tracking-wide text-red-700 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-45'
                      }
                    >
                      Отменить заказ
                    </button>
